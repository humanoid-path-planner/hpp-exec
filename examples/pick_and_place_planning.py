...
Run standalone to plan and inspect the result:
    python3 ~/devel/hpp-exec/examples/pick_and_place_planning.py

Or import from another script to access the path, robot, and graph:
    from pick_and_place_planning import plan_pick_and_place
...
def extract_configs(path, n_per_unit=50):
    """Sample configs from an HPP path.

    Returns (full_configs, arm_configs, times) where:
    - full_configs: list of full config vectors (16 DOF)
    - arm_configs: list of arm-only configs (7 DOF)
...
def main():
    ...
    print(f"  {len(arm_configs)} configs extracted")
    print(f"  Start cube pos: {full_configs[0][9:12]}")
    print(f"  End cube pos:   {full_configs[-1][9:12]}")
